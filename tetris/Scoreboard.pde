class Scoreboard {
  private final int SCOREBOARD_WIDTH;
  ...
  private final int SCOREBOARD_X_BUFFER = Playfield.PLAYFIELD_BLOCK_WIDTH * Tetris.PLAYFIELD_BLOCK_SCALE + 1;

  Scoreboard(Tetris tetris, int width, int height) {
    this.tetris = tetris;
    
    this.SCOREBOARD_WIDTH = width;
    this.SCOREBOARD_HEIGHT = height;
 ...
 void drawNextPiece(Piece piece) {
   Playfield playfield = tetris.getPlayfield();
   PImage blockImage = playfield.getBlockImages()[piece.getType()];
   
   int blockWidth = playfield.getBlockWidth();
   int padding = 20;
